...
// Throttled mouse move: queue only last position
...
ipcMain.on('mouse-move', (_e, x: number, y: number) => {
  ...
  if (!moveTimer) {
    moveTimer = setTimeout(async () => {
      moveTimer = null;
      try {
        await mouse.setPosition({ x: moveX, y: moveY });
      } catch (err) {
...
ipcMain.on('mouse-click', async (_e, button: number) => {
  try {
    const btn = button === 2 ? Button.RIGHT : Button.LEFT;
...
// Map scancode to nut-js Key enum
const keycodeToKey: Record<number, Key> = {
  ...
  27: Key.RightBracket, 43: Key.Backslash,
  41: Key.Grave,
};
...
async function createWindow() {
  mouse.config.autoDelayMs = 0; // Remove delays for responsiveness
