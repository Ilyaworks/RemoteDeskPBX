  ...
  socket.on('message', (raw) => {
    try {
      ...
      if (type === 'join-viewer') {
        ...
        session.ws.send(JSON.stringify({ type: 'viewer-joined', roomId }));
        return;
      }
  ...
  socket.on('close', () => {
    for (const [roomId, s] of sessions) {
      ...
      if (s.viewer === socket) {
        ...
        console.log(`ℹ️  Viewer left room ${roomId}`);
        return;
      }
    }
  });
...
fastify.listen({ port, host: '0.0.0.0' }).then(() => {
  console.log('════════════════════════════════════════');
  console.log('  RemoteDeskPBX SERVER');
  console.log('  Port: ' + port);
  console.log('  URL: ws://YOUR_IP:' + port + '/ws');
  console.log('  Give this URL to your friend');
  console.log('════════════════════════════════════════');
